...
async function generate300AppReport(filename) {
    ...
    const generateFunctional = () => features.flatMap(f => [
        `Verify successful execution of ${f} with valid inputs on Android`,
        `Verify appropriate error message for ${f} with invalid inputs`,
        `Verify ${f} behavior when internet connection is lost`,
        `Verify state persistence for ${f} after app restart in emulator`
    ]);
    ...
    for (const [category, cases] of Object.entries(rawCases)) {
        ...
        for (let i = 0; i < 60; i++) {
            ...
            let tcName = cases[i] || `Verify edge case ${i} for ${category}`; 
    ...
    console.log(`Generated ${filename} with ${testCases.length} Android E2E test cases!`);
}
